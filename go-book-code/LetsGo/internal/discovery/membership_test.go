...
func (h *handler) Join(id, addr string) error {
	if h.joins != nil {
		// track how many times our Membership calls the handler's Join
...
func (h *handler) Leave(id string) error {
	if h.leaves != nil {
		// track how many times our Membership calls the handler's Leave
		h.leaves <- id
	}
	return nil
}
...
func setupMember(t *testing.T, members []*Membership) ([]*Membership, *handler) {
	...
	h := &handler{}
	if len(members) == 0 {
		// this member is first member
		h.joins = make(chan map[string]string, 3)
		h.leaves = make(chan string, 3)
	} else {
		// the cluster has existing members, this member need to join the cluster
