...
select t.team_id, t.team_name, t.num_points
from (
    select t.team_id, t.team_name, COALESCE(score.num_points, 0) num_points from teams t
    left join (
        select score.team_id, sum(win)+sum(draw) as num_points from (
            select host.host_team as team_id, 3*count(case when is_win then 1 end) as win, count(case when is_draw then 1 end) as draw from (
                select host_team, (host_goals > guest_goals) as is_win, (host_goals = guest_goals) as is_draw, (host_goals < guest_goals) as is_lose
                from matches
            ) host 
            group by host.host_team
            union
            select guest.guest_team as team_id, 3*count(case when is_win then 1 end) as win, count(case when is_draw then 1 end) as draw from (
                select guest_team, (host_goals < guest_goals) as is_win, (host_goals = guest_goals) as is_draw, (host_goals > guest_goals) as is_lose
                from matches
            ) guest 
            group by guest.guest_team
        ) score
        group by score.team_id
    ) score
    on t.team_id = score.team_id
) t
order by t.num_points desc, t.team_id asc

---
result https://app.codility.com/demo/results/training3Y9P42-DC4/